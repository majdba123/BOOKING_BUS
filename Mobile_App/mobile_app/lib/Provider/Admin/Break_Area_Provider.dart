...
class BreakAreaProvider with ChangeNotifier {
  ...
  Future<void> fetchBreakAreas(String accessToken, var breakId) async {
    ...
    try {
      _BreakAreas =
          await BreakAreaApiService().fetchBreakArea(accessToken, breakId);
    } catch (e) {
  ...
  Future<String> addBreakArea(
    String accessToken,
    var areaId,
    String name,
  ) async {
    String message =
        await BreakAreaApiService().AddBreakArea(accessToken, areaId, name);
    notifyListeners();
    return message;
  }

  Future<void> updateBreakArea(String token, String id, String name) async {
    try {
      final updatedBreakArea =
          await BreakAreaApiService().updateBreakArea(token, id, name);
  ...
  Future<void> deleteBreakArea(String token, String id) async {
    try {
